...
Long_t MUSEteleTracker::process_efficiency()
{
    ...
    //Ishara definitions
    std::vector< std::vector<int> > IBHhits(Hits->hits.size(), std::vector<int>(2));
    std::vector< std::vector<double> > IBHhitxyz(Hits->hits.size(), std::vector<double>(3));
	...
	BHhitcounts = BHhitcounts + 1;
	//printf("BH plane and bar are  (%d,%d)\n",BHhits[i][0],BHhits[i][1]);
	//printf("BH hit coordinates are  (%7.4f,%7.4f,%7.4f)\n",BHhitxyz[i][0],BHhitxyz[i][1],BHhitxyz[i][2]);        
    ...
    //{
        ...
        //int plane, side, bar;
        //BH_internal_to_logic(Hits->hits[0].id,&plane,&bar,&side);
    ...
    //Here is the BH event 1 test closing point. Moving this to the end of the code
