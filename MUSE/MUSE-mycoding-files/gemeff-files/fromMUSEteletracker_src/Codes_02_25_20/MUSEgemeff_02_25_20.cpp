        ...
        for (int gem1=0; gem1<NG; gem1++)
              {
                ...
                if (gem1!=gem0)
                {
                  ...
                  //double cluster_charge_gem1[2]={0.0,0.0};
              ...
              //if ((fabs(xg[gemID[0]])<50)&&(fabs(yg[gemID[0]])<50)&&(fabs(xg[gemID[1]])<50)&&(fabs(yg[gemID[1]])<50))
              //{
                //printf("GEM IDs %d) \n",gemID);
                /*
                ...
                */
                 ...
                 if (gemID[0]!=gemID[1])
                 {
                  ...
                  if ((fabs(xg[gemID[0]])<50.0)&&(fabs(yg[gemID[0]])<50.0)&&(fabs(xg[gemID[1]])<50.0)&&(fabs(yg[gemID[1]])<50.0))
                 {
                ...
                //printf("Slope y = %5.2f \n",slopeyg);
                  ...
                  {
                    //printf("outside the active area\n");
                    //printf("Coordinate x = %5.2f ,%5.2f \n",xg[gemID[0]],xg[gemID[1]]);
                    //printf("Coordinate y = %5.2f ,%5.2f \n",yg[gemID[0]],yg[gemID[1]]);
                  }
                 ...
                 //Project the tracks on BH planes
                    ...
                    double BHplane2X = xg[0] + slopexg*(Plane2crds[2]-zgempos[gemID[0]]);
                    double BHplane2Y = yg[0] + slopeyg*(Plane2crds[2]-zgempos[gemID[0]]);

                    double BHplane3X = xg[0] + slopexg*(Plane3crds[2]-zgempos[gemID[0]]);
                    double BHplane3Y = yg[0] + slopeyg*(Plane3crds[2]-zgempos[gemID[0]]);  
                    ...
                    */
                    ...
                    /*
                      double BHplane2X = Plane2crds[0] + slopexg*(zgempos[gemID[1]]-zgempos[gemID[0]]);
                      double BHplane2Y = Plane2crds[1] + slopeyg*(zgempos[gemID[1]]-zgempos[gemID[0]]);

                      double BHplane3X = Plane3crds[0] + slopexg*(zgempos[gemID[1]]-zgempos[gemID[0]]);
                      double BHplane3Y = Plane3crds[1] + slopeyg*(zgempos[gemID[1]]-zgempos[gemID[0]]);  
                   ...
                   //printf("Projected BH plane %d : hit coordinates are  (%7.4f,%7.4f,%7.4f)\n",Plane3,BHplane3X,BHplane3Y,Plane3crds[2]);
                    ...
                    H1(BHdy, Form("Efficiency/%s/BH_residuals_of_gem_%s_Y", gemdet, gemdet), Form("Residual Y-distribution of Tracks Projected on BH to evaluate %s GEM ", gemdet),50,-50.0,50.0);
                     ...
                     if((fabs(BHdx)<5.0000) && (fabs(BHdy)<5.0000))
                      {
