  ...
  if(BHPlaneCount[2]==1 and BHPlaneCount[3]==1)
  {
    ...
    {   
      ...
      for (int gem1=0; gem1<NG; gem1++)
      {
        ...
        if (gem1==gem0) // Not considering the GEM0
        {
         continue; 
        };
         //printf("GEM1 ID = %d\n",gem1);
        ...
        if((good_cluster_gem1==false) and (fabs(xg[gemID[gem1count]])<50.0)and(fabs(yg[gemID[gem1count]])<50.0) ) continue;
      ...
      //if ((fabs(xg[gemID[0]])<50.0)and(fabs(yg[gemID[0]])<50.0))
      //{  
      ...
      if((fabs(BHdx)>6.0) or (fabs(BHdy)>6.0)) continue;
      ...
      //if((fabs(xe[gem0])>50.0) or (fabs(ye[gem0])>50.0)) continue;
      ...
      //}; //if clusters on two gems are inside active area
