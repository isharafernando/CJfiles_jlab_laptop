...
Long_t MUSEteleTracker::process_efficiency()
{
    ...
    //const double zgempos[3]= { -536.0, -474.0 , -412.0 }; //values measured in 2018 at PSI
    int zgemd=0;
...
//Define slopes here (changed by Ishara on 02/14/2020)
// Changed zgempos to zgem0, and gem0 to gem
						double slopexe = (xe[2]-xe[1])/(zgem0[gemt]-zgem0[gem]);
						double slopeye = (ye[2]-ye[1])/(zgem0[gemt]-zgem0[gem]);


//BH Filter conditions
						//Project the tracks on BH planes
                          double BHplane2X = Plane2crds[0] + slopexe*(zgem0[zgemd]-zgem0[gem]);
                          double BHplane2Y = Plane2crds[1] + slopeye*(zgem0[zgemd]-zgem0[gem]);

 						  double BHplane3X = Plane3crds[0] + slopexe*(zgem0[zgemd]-zgem0[gem]);
                          double BHplane3Y = Plane3crds[1] + slopeye*(zgem0[zgemd]-zgem0[gem]);  
 				 ...
 				 if((fabs(BHdx)<1.0000) && (fabs(BHdy)<1.0000))
  						{
									...
									if ((found_gem2_gdcluster)&&((fabs(xe[2])<50) && (fabs(ye[2])<50)))  // If the maximum charge cluster found on the second GEM at loop 2
									{
									  ...
									  //std::cout << "MS" << std::endl;
										...
										//std::cout << "US" << std::endl;
									  	...
									  	double X3 = xe[1] + slopexe*(zgem0[zgemd]-zgem0[gem]);
									  	double Y3 = ye[1] + slopeye*(zgem0[zgemd]-zgem0[gem]); 
