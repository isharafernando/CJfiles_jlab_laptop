...
//	  }
    ...
    for (int gems=0; gems<GEM_NUM; gems++)
    {
		...
		//printf ("Ishara version: max charge x: %5.2lf \n", xmaxchg[1]);
            }
            else test=false;
        };
...
for (int gems=0 ; gems<3; gems++)
    {
    	...
    	for (unsigned int g1=0; g1<clusters->hits.size(); g1++)
    	{
    		...
    		if(cluster_gem_charge[gemanyhit[gems]]>maxcharge) {
    			...
    			//printf ("Jesmin version: max charge x: %5.2lf \n", xmaxchg[1]);
    		}
    		else test=false;
    	};
    	...
    	if ((gems==0)&&test) H2(xmaxchg[1],ymaxchg[1], Form("Efficiency/US/max_charge_clusters_USGEM_Jesmin"), Form("Maximum Charge Cluster positions on US GEM"),50,-50.0,50.0,50,-50.0,50.0);
    	if ((gems==1)&&test) H2(xmaxchg[1],ymaxchg[1], Form("Efficiency/4TH/max_charge_clusters_4THGEM_Jesmin"), Form("Maximum Charge Cluster positions on 4TH GEM"),50,-50.0,50.0,50,-50.0,50.0);
    	if ((gems==2)&&test) H2(xmaxchg[1],ymaxchg[1], Form("Efficiency/MS/max_charge_clusters_MSGEM_Jesmin"), Form("Maximum Charge Cluster positions on MS GEM"),50,-50.0,50.0,50,-50.0,50.0);
    };
	    ...
	    //printf("found one or more clusters on GEM0\n");
            ...
            if(good_cluster_gem0)
            {
			  ...
			  //Project the tracks on BH planes
                          //double BHplane2X = Plane2crds[0] + slopexe*(zgempos[zgemd]-zgempos[gem0]);
                          //double BHplane2Y = Plane2crds[1] + slopeye*(zgempos[zgemd]-zgempos[gem0]);

			  //double BHplane3X = Plane3crds[0] + slopexe*(zgempos[zgemd]-zgempos[gem0]);
                          //double BHplane3Y = Plane3crds[1] + slopeye*(zgempos[zgemd]-zgempos[gem0]);  
