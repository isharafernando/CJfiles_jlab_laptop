...
Long_t MUSEteleTracker::process_efficiency()
{
  ...
  H1(BHPlaneCount[2], Form("Efficiency/BH_Plane2_Cluster_Multiplicity"), Form("Cluster Multiplicity of BH Plane 2"),11,-0.5,10.5);
  H1(BHPlaneCount[3], Form("Efficiency/BH_Plane3_Cluster_Multiplicity"), Form("Cluster Multiplicity of BH Plane 3"),11,-0.5,10.5);
  ...
  if(BHPlaneCount[2]==1 and BHPlaneCount[3]==1)
  {
    ...
    H1(BHPlaneCountAC[2], Form("Efficiency/BH_Plane2_1Hitluster_Multiplicity"), Form("Cluster Multiplicity of BH Plane 2 with 1 hit"),11,-0.5,10.5);
    H1(BHPlaneCountAC[3], Form("Efficiency/BH_Plane3_1Hit_Cluster_Multiplicity"), Form("Cluster Multiplicity of BH Plane 3 with 1 hit"),11,-0.5,10.5);
    ...
    H1(gemmultiafter[0], Form("Efficiency/US/BH_filtered_Cluster_Multiplicity_US"), Form("Cluster Multiplicity of US GEM after BH cluster filtering"),11,-0.5,10.5);
    H1(gemmultiafter[1], Form("Efficiency/4TH//BH_filteredCluster_Multiplicity_4TH"), Form("Cluster Multiplicity of 4TH GEM after BH cluster filtering"),11,-0.5,10.5);
    H1(gemmultiafter[2], Form("Efficiency/MS//BH_filteredCluster_Multiplicity_MS"), Form("Cluster Multiplicity of MS GEM after BH cluster filtering"),11,-0.5,10.5);
    ...
    for (int gem0=0; gem0<NG; gem0++)  
    {
      ...
      //printf("GEM IDs %d, %d \n",gemID[0],gemID[1]);
      //printf("(%5.2f,%5.2f)\n",xg[1],yg[1]);
      //printf("(%5.2f,%5.2f)\n",zgempos[gemID[1]],zgempos[gemID[0]]);
      //printf("(%5.2f,%5.2f)\n",zg[1],zg[0]);
      //printf("Slope x = %5.4f \n",slopexg);
      //printf("Slope y = %5.4f \n",slopeyg);
      ...
      //Her we use a negative sign because x and y should be flipped to get the MUSE coordinate frame
       ...
       H2(-BHplane3X,Plane3crds[0], Form("Efficiency/%s/BHPlane3_corr_X%s", gemdet,gemdet), Form("Correlation of BH Plane 3 X : ActualX vs ProjectedX: %s GEM", gemdet),50,-50.0,50.0,13,-50.0,50.0);
       
       H2(-BHplane3X,Plane3crds[1], Form("Efficiency/%s/BHPlane3_corr_XY%s", gemdet,gemdet), Form("Correlation of BH Plane 3 Y : ActualY vs ProjectedX: %s GEM", gemdet),50,-50.0,50.0,13,-50.0,50.0);
            
       H2(-BHplane3Y,Plane3crds[0], Form("Efficiency/%s/BHPlane3_corr_YX%s", gemdet,gemdet), Form("Correlation of BH Plane 3 X : ActualX vs ProjectedY: %s GEM", gemdet),50,-50.0,50.0,13,-50.0,50.0);
       
       H2(-BHplane3Y,Plane3crds[1], Form("Efficiency/%s/BHPlane3_corr_Y%s", gemdet,gemdet), Form("Correlation of BH Plane 3 Y : ActualY vs ProjectedY vs: %s GEM", gemdet),50,-50.0,50.0,13,-50.0,50.0);
      ...
      H1(BHdy, Form("Efficiency/%s/BH_residuals_of_gem_%s_Y", gemdet, gemdet), Form("Residual Y-distribution of Tracks Projected on BH to evaluate %s GEM ", gemdet),14,-50.0,50.0);
        ...
        if((fabs(BHdx)<16.0) and (fabs(BHdy)<16.0)) 
        {
