...
Long_t MUSEteleTracker::process_GEMBH_efficiency()
{
    ...
    int Plane0=0;
    int Plane0bar=0;
    double Plane0crds[3]={0,0,0};
    int Plane1=0;
    int Plane1bar=0;
    double Plane1crds[3]={0,0,0};
    int Plane2=0;
    int Plane2bar=0;
    ...
    int BHPlaneCt[4]={0,0,0,0};

    //printf("Event  %d ----> Number of BH Hits %d \n",event_num, Hits->hits.size());
    for(size_t i = 0; i < Hits->hits.size(); i++)
    {
       //printf("Event  %d ----> Number of BH Hit ID %d -------> In-time hits   %d\n",event_num, i,Hits->hits[i].tdc_hits[0].size());
       /*for(size_t q = 0; q < Hits->hits[i].tdc_hits[0].size(); q++)
       {
          int testplane = Hits->hits[i].wall_id;
          int testbar = Hits->hits[i].bar_id; 
          //printf("TDC hist ID %d\n",q);
          //printf("Event   %d:   BH hit ID  %d:  In-time hit ID   %d:   Wall ID    %d:   Bar ID    %d\n",event_num,i,q,testplane,testbar);
       } */
      ...
      //handle.BH[plane][bar]->LocalToMaster(loc,mas);
        ...
        //std::vector< std::vector<double> BHposition;
      int Intime_hits = Hits->hits[i].tdc_hits[0].size();
      if(Intime_hits==1){
         /*
        int testplane = Hits->hits[i].wall_id;
        int testbar = Hits->hits[i].bar_id; 
       
        printf("Event   %d:   BH hit ID  %d:   Wall ID    %d:   Bar ID    %d\n",event_num,i,testplane,testbar);
        */
        
        int check_plane = Hits->hits[i].wall_id;

        if(check_plane==0){
          BHPlaneCt[0]=BHPlaneCt[0]+1;
        }

        if(check_plane==1){
          BHPlaneCt[1]=BHPlaneCt[1]+1;
        }

        if(check_plane==2){
          BHPlaneCt[2]=BHPlaneCt[2]+1;
        }

        if(check_plane==3){
          BHPlaneCt[3]=BHPlaneCt[3]+1;
        }
         
      };
    };     
         //printf("Plane 0 :  %d;    Plane 1 :  %d;     Plane 2 :  %d;      Plane 3 :  %d; \n", BHPlaneCt[0],BHPlaneCt[1],BHPlaneCt[2],BHPlaneCt[3]);

  if((BHPlaneCt[0]==1 and BHPlaneCt[1]==1) or (BHPlaneCt[2]==1 and BHPlaneCt[3]==1)){ 
      ...
      for(size_t i = 0; i < Hits->hits.size(); i++)
     {
      ...
      if(Intime_hits==1){
        ...
        //printf("Event_no  %d   Hit_no   %d    Wall_ID   %d     Bar_ID    %d   # of in-time-hits   %d\n",event_num,i, plane, bar, Intime_hits);
        
        //debug(0,"BH hit size=%i\n",Hits->hits.size());
        ...
        { 
 	        Plane0=plane;
          Plane0bar = bar;
          Plane0crds[0] = mas[0]*10;
          Plane0crds[1] = mas[1]*10;
          Plane0crds[2] = mas[2]*10;
          //H1(Plane0crds[0], Form("GEMBHEfficiency/BH/Plane0/BH_Plane0_x_positions"), Form("Cluster x positions of BH Plane 0"),50,-50.0,50.0);
          //H1(Plane0crds[1], Form("GEMBHEfficiency/BH/Plane0/BH_Plane0_y_positions"), Form("Cluster y positions of BH Plane 0"),50,-50.0,50.0);
          //H1(Plane0crds[2], Form("GEMBHEfficiency/BH/Plane0/BH_Plane0_z_positions"), Form("Cluster z positions of BH Plane 0"),50,-675.0,-625.0);
          //H1(Plane0bar, Form("GEMBHEfficiency/BH/Plane0/BH_Plane0_bar_number"), Form("Bar number of BH Plane 0"),16,-0.5,16.5);
          BHPlaneCount[plane]++;
          //printf("%f\n",Plane0crds[2]);
        };
        ...
        { 
 	        Plane1=plane;
          Plane1bar = bar;
          Plane1crds[0] = mas[0]*10;
          Plane1crds[1] = mas[1]*10;
          Plane1crds[2] = mas[2]*10;
          //H1(Plane1crds[0], Form("GEMBHEfficiency/BH/Plane1/BH_Plane1_x_positions"), Form("Cluster x positions of BH Plane 1"),50,-50.0,50.0);
          //H1(Plane1crds[1], Form("GEMBHEfficiency/BH/Plane1/BH_Plane1_y_positions"), Form("Cluster y positions of BH Plane 1"),50,-50.0,50.0);
          //H1(Plane1crds[2], Form("GEMBHEfficiency/BH/Plane1/BH_Plane1_z_positions"), Form("Cluster z positions of BH Plane 1"),50,-650.0,-600.0);
          //H1(Plane1bar, Form("GEMBHEfficiency/BH/Plane1/BH_Plane1_bar_number"), Form("Bar number of BH Plane 1"),16,-0.5,16.5);
          BHPlaneCount[plane]++;
        }; 
        ...
        { 
          ...
          //H1(Plane2bar, Form("GEMBHEfficiency/BH/Plane2/BH_Plane2_bar_number"), Form("Bar number of BH Plane 2"),16,-0.5,16.5);
          BHPlaneCount[plane]++;
          //printf("Plane 2 coordinates : (%f,%f,%f)\n",Plane2crds[0],Plane2crds[1],Plane2crds[2]);
        }; 
       ...
       if(plane==3)
        { 
          ...
          //H1(Plane3crds[2], Form("GEMBHEfficiency/BH/Plane3/BH_Plane3_z_positions"), Form("Cluster z positions of BH Plane 3"),50,-600.0,-550.0);
          //H1(Plane3bar, Form("GEMBHEfficiency/BH/Plane3/BH_Plane3_bar_number"), Form("Bar number of BH Plane 3"),15,-0.5,14.5);
          BHPlaneCount[plane]++;
          //printf("Plane 3 coordinates : (%f,%f,%f)\n",Plane3crds[0],Plane3crds[1],Plane3crds[2]);
        };
      };
     };
        };
  ...
  H1(BHPlaneCount[0], Form("GEMBHEfficiency/BH/Plane0/BH_Plane0_Cluster_Multiplicity"), Form("Cluster Multiplicity of BH Plane 0"),11,-0.5,10.5);
  H1(BHPlaneCount[1], Form("GEMBHEfficiency/BH/Plane1/BH_Plane1_Cluster_Multiplicity"), Form("Cluster Multiplicity of BH Plane 1"),11,-0.5,10.5);
  ...
  if(BHPlaneCount[2]==1 and BHPlaneCount[3]==1)
  {
    ...
    int Plane0AC=0;
    int Plane0barAC=0;
    double Plane0crdsAC[3]={0,0,0};
    int Plane1AC=0;
    int Plane1barAC=0;
    double Plane1crdsAC[3]={0,0,0};
    int Plane2AC=0;
    int Plane2barAC=0;
    ...
    for(size_t i = 0; i < Hits->hits.size(); i++)
    {
      ...
      if(Intime_hits==1){
        ...
        { 
 	        Plane0=plane;
          Plane0barAC = bar;
          Plane0crdsAC[0] = mas[0]*10;
          Plane0crdsAC[1] = mas[1]*10;
          Plane0crdsAC[2] = mas[2]*10;
          H1(Plane0crdsAC[0], Form("GEMBHEfficiency/BH/Plane0/BH_Plane0_1hit_x_positions"), Form("Cluster x positions of BH Plane 0 1hit"),50,-50.0,50.0);
          H1(Plane0crdsAC[1], Form("GEMBHEfficiency/BH/Plane0/BH_Plane0_1hit_y_positions"), Form("Cluster y positions of BH Plane 0 1hit"),50,-50.0,50.0);
          H1(Plane0crdsAC[2], Form("GEMBHEfficiency/BH/Plane0/BH_Plane0_1hit_z_positions"), Form("Cluster z positions of BH Plane 0 1hit"),50,-675.0,-625.0);
          H1(Plane0barAC, Form("GEMBHEfficiency/BH/Plane0/BH_Plane0_1hit_bar_number"), Form("Bar number of BH Plane 0 1hit"),16,-0.5,16.5);
        ...
        { 
 	        Plane1=plane;
          Plane1barAC = bar;
          Plane1crdsAC[0] = mas[0]*10;
          Plane1crdsAC[1] = mas[1]*10;
          Plane1crdsAC[2] = mas[2]*10;
          H1(Plane1crdsAC[0], Form("GEMBHEfficiency/BH/Plane1/BH_Plane1_1hit_x_positions"), Form("Cluster x positions of BH Plane 1 1hit"),50,-50.0,50.0);
          H1(Plane1crdsAC[1], Form("GEMBHEfficiency/BH/Plane1/BH_Plane1_1hit_y_positions"), Form("Cluster y positions of BH Plane 1 1hit"),50,-50.0,50.0);
          H1(Plane1crdsAC[2], Form("GEMBHEfficiency/BH/Plane1/BH_Plane1_1hit_z_positions"), Form("Cluster z positions of BH Plane 1 1hit"),50,-650.0,-600.0);
          H1(Plane1barAC, Form("GEMBHEfficiency/BH/Plane1/BH_Plane1_1hit_bar_number"), Form("Bar number of BH Plane 1 1hit"),16,-0.5,16.5);
          BHPlaneCountAC[plane]++;
        }; 
       ...
       if(plane==3)
        { 
          ...
          H1(Plane3crdsAC[2], Form("GEMBHEfficiency/BH/Plane3/BH_Plane3_1hit_z_positions"), Form("Cluster z positions of BH Plane 3 1hit"),50,-600.0,-550.0);
    ...
    H1(BHPlaneCountAC[0], Form("GEMBHEfficiency/BH/Plane0/BH_Plane0_1Hitluster_Multiplicity"), Form("Cluster Multiplicity of BH Plane 0 with 1 hit"),11,-0.5,10.5);
    H1(BHPlaneCountAC[1], Form("GEMBHEfficiency/BH/Plane1/BH_Plane1_1Hit_Cluster_Multiplicity"), Form("Cluster Multiplicity of BH Plane 1 with 1 hit"),11,-0.5,10.5);
    ...
    for (int gem0=0; gem0<NG; gem0++)  
    {
      ...
      for(size_t i = 0; i < Hits->hits.size(); i++)
      {
        ...
        { 
          Plane0AC=plane;
          Plane0barAC = bar;
          H1(Plane0barAC, Form("GEMBHEfficiency/BH/Plane0/BH_Plane0_1hit_bar_after_projecting_track"), Form("Bar number of BH Plane 0 1hit after projecting track"),16,-0.5,16.5);
        ...
        { 
          Plane1AC=plane;
          Plane1barAC = bar;
          H1(Plane1barAC, Form("GEMBHEfficiency/BH/Plane1/BH_Plane1_1hit_bar_after_projecting_track"), Form("Bar number of BH Plane 1 1hit after projecting track"),16,-0.5,16.5);
      ...
      //Her we use a negative sign because x and y should be flipped to get the MUSE coordinate frame
       ...
       H2(BHplane3X,Plane3crds[0], Form("GEMBHEfficiency/%s/BHPlane3_corr_X%s", gemdet,gemdet), Form("Correlation of BH Plane 3 X : ActualX vs ProjectedX: %s GEM", gemdet),50,-50.0,50.0,12,-50.0,50.0);
       
       H2(BHplane3X,Plane3crds[1], Form("GEMBHEfficiency/%s/BHPlane3_corr_XY%s", gemdet,gemdet), Form("Correlation of BH Plane 3 Y : ActualY vs ProjectedX: %s GEM", gemdet),50,-50.0,50.0,12,-50.0,50.0);
            
       H2(BHplane3Y,Plane3crds[0], Form("GEMBHEfficiency/%s/BHPlane3_corr_YX%s", gemdet,gemdet), Form("Correlation of BH Plane 3 X : ActualX vs ProjectedY: %s GEM", gemdet),50,-50.0,50.0,12,-50.0,50.0);
       
       H2(BHplane3Y,Plane3crds[1], Form("GEMBHEfficiency/%s/BHPlane3_corr_Y%s", gemdet,gemdet), Form("Correlation of BH Plane 3 Y : ActualY vs ProjectedY vs: %s GEM", gemdet),50,-50.0,50.0,12,-50.0,50.0);
      ...
      H1(BHdy, Form("GEMBHEfficiency/%s/BH_residuals_of_gem_%s_Y", gemdet, gemdet), Form("Residual Y-distribution of Tracks Projected on BH to evaluate %s GEM ", gemdet),14,-50.0,50.0);
        ...
        //  H1(Plane3crds[0], Form("GEMBHEfficiency/BH/Plane3/BH_Plane3_x_positions_After_tracks"), Form("Cluster x positions of BH Plane 3 after projecting tracks"),50,-50.0,50.0);


        double BH_acceptance=8.0;
        if((fabs(BHdx)<BH_acceptance) and (fabs(BHdy)<BH_acceptance)) 
        {
         ...
         for (unsigned int g0=0; g0<clusters->hits.size(); g0++)
          ...
          if(!good_cluster_gem0) continue;
           ...
           dy=ye[gem0]-Y3; //hosrizontal residue
            ...
            double GEM_acceptance=10.0; 
            if((fabs(dx)<GEM_acceptance) and (fabs(dy)<GEM_acceptance) and (fabs(xe[gem0])<50.0) and (fabs(ye[gem0])<50.0))
          {
            ...
            H1(dx, Form("GEMBHEfficiency/%s/x_residue%s", gemdet, gemdet), Form("X residue on %s GEM", gemdet),2*fabs(GEM_acceptance),-GEM_acceptance,GEM_acceptance);
                          
            H1(dy, Form("GEMBHEfficiency/%s/y_residue%s", gemdet, gemdet), Form("Y residue on %s GEM", gemdet),2*fabs(GEM_acceptance),-GEM_acceptance,GEM_acceptance);
           };
