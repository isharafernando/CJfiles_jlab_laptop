...
Long_t MUSEteleTracker::process_efficiency()
{
    ...
    for (int gems=0; gems<GEM_NUM; gems++)
    {
        ...
        if (gems==1)  multiplicityMS->Fill(gemanyhit[gems]);
                    ...
                    // If the maximum charge cluster found on the third GEM at loop 3
                    {
                        ...
                        //Project tracks on the fourth GEM (if all  both first, second and third GEMs have only maximum charge cluster selected)
                          ...
                          H1(Y3, Form("Efficiency/%s/tracksprojectedgem%sY_any", gemdet, gemdet), Form("Tracks Projected on %s GEM Y-Any cluster", gemdet),50,-50.0,50.0);
                                ...
                                /*projectedtracksXY=dH2(Form("Efficiency/%s/tracksprojectedgem%s_any", gemdet,gemdet), Form("Tracks Projected on %s GEM-Any cluster", gemdet),50,-50.0,50.0,50,-50.0,50.0);
                                
                                projectedtracksX=dH1(Form("Efficiency/%s/tracksprojectedgem%sX_any", gemdet, gemdet), Form("Tracks Projected on %s GEM X-Any cluster", gemdet),50,-50.0,50.0);
                                
                                projectedtracksY=dH1(Form("Efficiency/%s/tracksprojectedgem%sY_any", gemdet, gemdet), Form("Tracks Projected on %s GEM Y-Any cluster", gemdet),50,-50.0,50.0);
                                
                                projectedtracksXY->Fill(X3,Y3);
                                projectedtracksX->Fill(X3);
                                projectedtracksY->Fill(Y3);
                               */
