...
MY_BOOL mpdIntRunning = FALSE;	/* running flag */
...
int mpdApvGetI2C(int id, int ia) { return fApv[id][ia].i2c; };
 ...
 */
int mpdFIFO_ReadSingle(int id, int channel,	// apv channel (FIFO)
...
int mpdFIFO_GetTotalNwords_and_DisableTrigger(int id, unsigned int *latch_addr)
{
    int k = 0;
    int nwords = 0;
    int totalwords = 0;
    ...
    for (k=0; k<fMpd[id].nAPV; k++) {
        ...
        while( (nwords <=0 ) && (i <= max_retry) ) {
            if( max_retry > 0 ) i++;
            mpdFIFO_GetNwords(id, fApv[id][k].adc, &nwords);
        }
        totalwords += nwords;
#ifdef DEBUG
        //printf("MPD TRIG LEMO = %d\n", mpdGetInPath(id, MPD_IN_FRONT, MPD_IN_TRIG));
#endif
        int counter = 0;
        if (nwords>=130 && counter < 1) { // at least 130 words in fifo for one APV
            //if (mpdGetInPath(id, MPD_IN_FRONT, MPD_IN_TRIG) != 0) mpdSetInPath(id, MPD_IN_FRONT, MPD_IN_TRIG, 0);
            vmeWrite16(latch_addr, 1);  // Turn on microbusy latch = Disable the trigger
            counter++;
#ifdef DEBUG
            //printf("MPD TRIG LEMO = %d\n", mpdGetInPath(id, MPD_IN_FRONT, MPD_IN_TRIG));
            //printf("Front panel disabled\n");
#endif
            //mpdTRIG_PauseEnableQuick(id);
            //usleep(500);
        }
        //printf(" FIFO=%d, words=%d\n", k, nwords);
    }
#ifdef DEBUG
    MPD_DBG(" counts= %d, total words=%d\n", i, totalwords);
#endif
    return totalwords;
}


int mpdFIFO_GetTotalNwords(int id)
{
    int k = 0;
    int nwords = 0;
    int totalwords = 0;
    ...
    for (k=0; k<fMpd[id].nAPV; k++) {
        ...
        while( (nwords <=0 ) && (i <= max_retry) ) {
            if( max_retry > 0 ) i++;
            mpdFIFO_GetNwords(id, fApv[id][k].adc, &nwords);
            //printf("%s: Number of words to be read %d   %d\n",__FUNCTION__,nwords, success);
...
int mpdPED_Write(int id, int ch, int v)	// ch = 0..7
{
  int i, data[128];
...
int mpdPED_Read(int id, int ch, int *ped_even, int *ped_odd)	// TBD
{
  int i;
...
int mpdTHR_Write0(int id, int ch, int *thr_even, int *thr_odd)	// ch = 0..7
{
  int success = OK, i;
...
int mpdTHR_Write(int id, int ch, int v)	// ch = 0..7
{
  int i, data[128];
...
int mpdTHR_Read(int id, int ch, int *thr_even, int *thr_odd)	// TBD
{
  int i;
