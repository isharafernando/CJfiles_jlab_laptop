...
def deltaOSPlus(filename,y,xp):
    ...
    for i in x:
        ...
        for j in range(1,nnn+1):
            ...
            if tempvvv01<0 and tempvvv02<0:
                #print(i,'both are negative: upper bound')
                negativecountx.append(i)
            tempvvv2=np.square(tempvvv1)            
            tempsum=tempsum+tempvvv2
        tempvvv.append(np.sqrt(tempsum))
    print negativecountx    
    return tempvvv
    
def deltaOSMinus(filename,y,xp):
    test_files = glob.glob('./Data/par_'+filename+'/*.par')
    #print test_files[0]
    nn=len(test_files)
    nnn=(nn-1)/2
    ...
    for i in x:
        ...
        for j in range(1,nnn+1):
            ...
            tempvvv1=max(tempvvv01,tempvvv02,0)
            if tempvvv01<0 and tempvvv02<0:
                #print(i,'both are negative: lower bound')
                negativecountx.append(i)
            tempvvv2=np.square(tempvvv1)            
            tempsum=tempsum+tempvvv2
        tempvvv.append(np.sqrt(tempsum))
...
#def OffShellPlot(PlotNot,Outlists,OutFileNames,Parlists,ParFileNames,filenames,lists,DataFileNames,ymin,ymax,ALabels):
#    #x1=np.linspace(0,1,100)
#    x=PlotData(lists,lists[1],DataFileNames[1],2,9)[0]
#    ux=PlotData(lists,lists[1],DataFileNames[1],2,9)[1]
#    ubx=PlotData(lists,lists[1],DataFileNames[1],2,13)[1]
#    dx=PlotData(lists,lists[1],DataFileNames[1],2,7)[1]
#    dbx=PlotData(lists,lists[1],DataFileNames[1],2,15)[1]
#    deltauxubx=ux-ubx
#    deltadxdbx=dx-dbx
#    print len(x)
#    #print x1
#    print len(deltauxubx)    
...
#    dy1p=deltaOSPlus(filenames[0],1,x)
#    dy1m=deltaOSMinus(filenames[0],1,x)
...
#                tempsumruleintux=temp1*deltauxubx
#                tempsumruleintdx=temp1*deltadxdbx
#                print ioff9check
#            #temp1=OffShelliOff12(x,Nvalues[i],x0values[i],x1values[i])    
#            #iOff12
#            #print lineoff4(ParFileNames[i+1])
#            temp1yp=deltaOSPlus(filenames[i+1],2,x)
#            temp1ym=deltaOSMinus(filenames[i+1],2,x)
...
#            tempsumruleintux=temp1*deltauxubx
#            tempsumruleintdx=temp1*deltadxdbx
#            #print tempsumruleint
#            #print lineoff4(ParFileNames[i+1])
#            #plt.plot(x,temp2,'-',label=ALabels[i],linewidth=LW,c=line_colors[i])
#            temp2yp=deltaOSPlus(filenames[i+1],2,x)
#            temp2ym=deltaOSMinus(filenames[i+1],2,x)
#            #print len(temp2)
#            #print len(temp2ym)
#            plt.plot(x,temp2,'-',label=ALabels[i],linewidth=LW,c=line_colors[i])
#            if PlotNot[i+1]==1:
...
#    tempsumruleux=sc.integrate.trapz(tempsumruleintux,x)
#    tempsumruledx=sc.integrate.trapz(tempsumruleintdx,x)
#    print tempsumruleux,tempsumruledx
...
def OffShellPlot(PlotNot,Outlists,OutFileNames,Parlists,ParFileNames,filenames,lists,DataFileNames,ymin,ymax,ALabels):
    ...
    dy1m=deltaOSMinus(filenames[0],1,x)
    for i in range(0,len(ParFileNames)-1):
        testdeg=lineoff4(ParFileNames[i+1])
        print(testdeg)
        if lineoff4(ParFileNames[i+1])==2:
            templist=[]
            for line in open(ParFileNames[i+1]):
            ...
            ioff9check=templist[ioffline][1][2]
            print ioff9check
            orderchecklines=parline(ParFileNames[i+1],'off')
            ordercheck=templist[orderchecklines[0]][-1]
            print ordercheck
#            #print templist[58]    
            if ioff9check=='9':
                temp1=OffShell(x,Nvalues[i],x0values[i],x1values[i])
                tempsumruleintux=temp1*deltauxubx/x
                tempsumruleintdx=temp1*deltadxdbx/x
                #print ioff9check
            else:
                temp1=OffShelliOff12(x,Nvalues[i],x0values[i],x1values[i])
                #tempsumruleintux=temp1*deltauxubx
            ...
            temp2=OffShelliOff13(x,Nvalues[i],x0values[i],x1values[i],x2values[i])
            #print temp2
            tempsumruleintux=temp2*deltauxubx/x
            tempsumruleintdx=temp2*deltadxdbx/x
            #print tempsumruleint
            #print lineoff4(ParFileNames[i+1])
            #plt.plot(x,temp2,'-',label=ALabels[i],linewidth=LW,c=line_colors[i])
            temp2yp=deltaOSPlus(filenames[i+1],2,x)
    ...
    plt.xlabel('$x$',fontsize='10')
    plt.ylabel('$\delta f^N$',fontsize='10')
    tempsumruleux=scipy.integrate.trapz(tempsumruleintux,x)
    tempsumruledx=scipy.integrate.trapz(tempsumruleintdx,x)
    print tempsumruleux,tempsumruledx
