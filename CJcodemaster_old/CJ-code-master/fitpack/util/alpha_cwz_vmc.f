...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      IMPLICIT NONE
      double precision alpha_S,XMU2,XLQCD5,get_alphaS,alfas5
      integer IALPHAs,NEFF,get_Nf
      integer ias
      common /alphas/ias
      ALPHA_S = 0.2
      NEFF=5
      ...
      if (ias.eq.0) then
         ...
         ! JAM routine by NS
         ! NOTE: self initializing, but needs to set_alphasmz()
         !       every time alphas(MZ) changes
         ALPHA_S=get_alphaS(XMU2)
         NEFF=get_Nf(XMU2)
      else
         print*, 'ERROR(ALPHA_S): ias out of range:',ias
      end if
...
c      Real*8 mu
c      INTEGER ILOOP,NEFF
      Integer ILOOP,NEFF,IDUM,IORD,NMAX,IVL
      COMMON/INPUT/IDUM,IORD,NMAX,IVL,XMC,XMB,NCB
      A  = 1.00
c      MB = 4.750
c      mb=5.0
c       mb=4.5
...
c      mc=1.6
c      mc=1.3
c  mc, mb now set in altpfit10 or read in from .par file
c
      AMB=A*xMB
      AMC=A*xMC
      MU=SQRT(Q2)
