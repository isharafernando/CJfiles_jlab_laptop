...
******************************************
      ...
      double precision y,ymin,ymax,deltay,gamma1,gamma2,gamma3,gamma4
     &     ,f1,f2,f3,f4,fbj
     &     ,norm1,norm2,norm3,norm4,normbj,yDmin,yDmax

      double precision phi_int2d
...
C                          ! 3=LC
      ...
      write(6,200) '   y      g=',gamma1,'     g=',gamma2
     &     ,'     g=',gamma3,'     g=',gamma4 
     &     ,'     g=',1.0,' ("Bj.limit")'
 200  format(A,5(F4.1,A))
      do i=0,ny
         y   = ymin+i*deltay
         f1  = phi_int2d(y,gamma1,ismear)
         f2  = phi_int2d(y,gamma2,ismear)
         f3  = phi_int2d(y,gamma3,ismear)
         f4  = phi_int2d(y,gamma4,ismear)
         fbj = phi_int2d(y,1d0,ismear)
         write(6,100) y,f1,f2,f3,f4,fbj
      end do
 100  format(f6.2,5(F11.5))
      ...
      DO I=1,NTERMS
         y=0.5*(yDmax-yDmin)*XI(I)+0.5*(yDmax+yDmin) ! y = yD in [x,1]
         norm1 = norm1+.5*(yDmax-yDmin)*WI(I)*phi_int2d(y,gamma1,ismear)
         norm2 = norm2+.5*(yDmax-yDmin)*WI(I)*phi_int2d(y,gamma2,ismear)
         norm3 = norm3+.5*(yDmax-yDmin)*WI(I)*phi_int2d(y,gamma3,ismear)
         norm4 = norm4+.5*(yDmax-yDmin)*WI(I)*phi_int2d(y,gamma4,ismear)
         normbj = normbj+.5*(yDmax-yDmin)*WI(I)*phi_int2d(y,1d0,ismear)
      ENDDO
      
      write(6,*) '-------------------------------------------------'
     &     //'-------------'
      write(6,300) norm1,norm2,norm3,norm4,normbj
 300  format('norm  ',5(F11.5))

      stop 
      end

      include 'phi_int2d.f'
      include '../util/trmstr.f'
