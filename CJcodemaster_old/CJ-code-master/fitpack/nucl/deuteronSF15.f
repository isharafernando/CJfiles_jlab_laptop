...
C...Convolution approximation (x used in convolution here is xD in [0,1])
      ...
      if(istruc.eq.-1) then
         ...
         BBA = 0
         GLUEA  = 0
         if (ioff.ge.5.and.ioff.le.8) then
            UAo = 0
            DAo = 0
            UBAo = 0
            DBAo = 0
      ...
      DO I=1,NTERMS
         ...
         if ((ioff.eq.0).or.(ioff.ge.2)) then 
            ...
            if (istruc.eq.0) then 
               ...
               if (ioff.ge.5.and.ioff.le.8) then ! (fm)KP off-shell model
               ...
               fy_off  = 0d0
               F2N = 0d0
               if (ioff.ge.5.and.ioff.le.8) then ! fmKP off-shell model
...
*                                                ! in y, gamma, xN=xB/y=xD/yD
            ...
            write(*,*) 'ERROR (smearf2-1): ioff out of range = ',ioff
            stop
         end if
         ...
         if (v(1).lt.0.995d0) then
            ...
            if (istruc.ne.-1) then ! Structure function convolution
               ...
               if (ioff.ge.5.and.ioff.le.8) then ! PDF-level offshell correction
                     FFdoff = FFdoff 
     &                      + weight*(foff_diag*FFNoff+foff_off*F2Noff)
               ...
               if (ioff.ge.5.and.ioff.le.8) then ! off-shell PDF convolution
...
*    ... PDF-level models (can be used with any observable)
      else if (ioff.ge.5.and.ioff.le.8) then
