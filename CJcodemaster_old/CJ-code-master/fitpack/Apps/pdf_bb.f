...
*             iht       = HT code
      ...
      integer ioff,idum1,idum2,idum3,idum4,idum5
...
*    *** reads QCD parameters from .par file
      ...
      read(nu,*) dummy,xmb
      read(nu,*) dummy,ncb
      read(nu,*,err=100) dummyhq,ihq
 
*    *** Reads theory corrections codes
      read(nu,*) dummy
 100  if (dummyhq.ne."ihq") ihq=0 ! for compatibility with CJ12 .par files
      read(nu,*) dummy,inuke
...
c jfo Count number of PDF parameters
      ...
      do j=1,99
!***  NOTE (AA - 29 Mar 2016):
!***  if changing the 99 to only consider a minor of the hessian alpha,
!***  breaks the calculation of observable uncertainties with altpfit.
...
c jfo      npdfpar = nfree   ! uses all free parameters for pdf errors
 ...
 33   if (ios.ne.0) then
     ...
     &        //'missing or wrong error matrix (Bevington alfa)'
      end if
...
c              endif
 ...
 25   format(5es12.5)
      ...
      subroutine writedat(datfile,type)
...
!     write(outn,*) '5 -5 4 -4 3 -3 2 -2 1 -1 21'      
      write(outn,'(A)') '-5 -4 -3 -2 -1 1 2 3 4 5 21'
