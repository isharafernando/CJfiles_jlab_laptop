...
*             iht       = HT code
      ...
      implicit none
      character*50 parfile,dummy,dummyhq
      integer nu,NextUn,i,j,inuke,iht,itmc,ios,nfree
      integer ioff,idum1,idum2,idum3,idum4,idum5,jmax
      integer CJ12flag
...
*     Other PDF parameters      ! Needed when using 'writepar'
      ...
      integer INS,NVL
      common/oparam1/INS,NVL
*     Choice of alpha_s routine
...
*    *** reads QCD parameters from .par file
      ...
      read(nu,*) dummy,xmb
      read(nu,*) dummy,ncb
      read(nu,*,err=100) dummy,ihq
 100  if (dummy.ne."ihq") then
         print '(/,A,/)', ' WARNING: likely using CJ12 .par file'
         CJ12flag=1
         ihq = 0
         ias = 0
      else 
         read(nu,*,err=101) dummy,ias
      end if
 101  if (dummy.ne."ias") then
         if (CJ12flag.eq.0) then
            print '(/,A,/)', ' WARNING: likely using CJ15 .par file'
         end if
         ias = 0
      else 
         read(nu,*) dummy
...
*    *** Reads theory corrections codes
      read(nu,*) dummy,inuke
      call split_nuke(inuke,idum1,idum2,idum3,ioff,idum4,idum5)
      ioff_flag=ioff      
      read(nu,*) dummy,itmc
      read(nu,*) dummy,iht

*    *** reads PDF params from file; stores in /curpar/
      DO J=1,100
         PAR(J)=0.
         uncrt(j)=0.
      end do
      ...
      do 31 j=1,99       ! only up to 99 because pname(100) is reserved
         ...
         params(j) = par(j)     ! par(j) ends up in common/curpar/ for use 
                                ! by other calculations (e.g. DIS13.f).
                                ! +++ MAYBE not NEEDED?? +++++
                                ! params(j) returned to the user who does
...
*    ...looks for offshell param
      call findoff(npar,par,pname,uncrt)
      call get_off(offpar)      

*     ... count number of PDF parameters to be used for error sets
*     ... NOTE: if you wish to only consider a subset of parameters
*     ...       do so *after* reading in the parfile in the calling routine
      npdfpar=0
      do j=1,99
...
c jfo      npdfpar = nfree   ! uses all free parameters for pdf errors
      do i=1,npdfpar
...
c jfo      do i=1,nfree
 ...
 33   if (ios.ne.0) then
         write(*,*) 'WARNING(readpdf): '
     &        //'missing or wrong Hessian matrix (Bevington alfa)'
      end if
...
*    *** reads in the cov matrix
 ...
 34   if (ios.ne.0) then
         write(*,*) 'WARNING(readpar): missing or wrong cov matrix'
      end if
...
*     QCD evolution and setup parameters
      double precision Q02,Q2MAX
      COMMON/Q2STUF/ Q02,Q2MAX
      integer ILOOP,IORD,NMAX,IVL,ncb
      double precision xmc,xmb
...
*     returns alpha_s at scale Q2, with Lambda_QCD, or alphas(MZ),
*     as read in from .par or .gf file.
*    
*     NOTE: simplified wrapper for ALPHA_S in util/alpha_cwz_vmc.f
*
*     INPUT:  Q2 [GeV^2]  = (dp) scale
*             iloop       = (i)  0=constant, 1=one loop, 2=two loops
*     OUTPUT: alpha_sa    = (dp) alpha_s
*             neff        = (i)  number of active flavors at scale Q2

      implicit none

      double precision alphas_sa,ALPHA_S
      ...
      alphas_sa = ALPHA_S(iloop,Q2,alambda5,neff)
...
c              endif
 ...
 25   format(5es12.5)
      ...
      subroutine writedat(datfile,type,flav)
...
C
      ...
      character*50 datfile,type
      character*1000 flav

      integer iQ,ix,i,j,len,outn,NextUn,nu2,neff
      real*8 x,Q,Q2,u0,d0,ub0,db0,cb0,sb0,bb0,glue
      real*8 lambda
...
*    *** x and Q grids
      integer nQ,nx,npartons
      parameter(nx=95) 
      parameter(nQ=26)
      parameter(npartons=11)
      double precision fullpdf(npartons*nx*nQ)      
      double precision Qgr(nQ), xgr(nx)
      data Qgr/   
...
*    *** Info for .info file
      ...
      do ix=1,nx
         ...
         do iQ=1,nQ
            Q=Qgr(iQ)
              ...
              fullpdf(i+1)  = dmax1(bb0,0d0)
              fullpdf(i+2)  = dmax1(cb0,0d0)
              fullpdf(i+3)  = dmax1(sb0,0d0)
              fullpdf(i+4)  = dmax1(ub0,0d0)
              fullpdf(i+5)  = dmax1(db0,0d0)
              fullpdf(i+6)  = dmax1(d0,0d0)
              fullpdf(i+7)  = dmax1(u0,0d0)
              fullpdf(i+8)  = dmax1(sb0,0d0)
              fullpdf(i+9)  = dmax1(cb0,0d0)
              fullpdf(i+10) = dmax1(bb0,0d0)
              fullpdf(i+11) = dmax1(glue,0d0)
      ...
      outn=NextUn()
      open (outn,FILE=datfile,status='new',err=200)
 ...
 31   format(es11.5,200es12.5)
      ...
      write(outn,32) fullpdf      
 32   format(es11.5,10es12.5)
      write(outn,'(A)') "---"
...
*    ... collects information for .info file
      ...
      goto 210
 200  print*, 'WARNING(writedat): datfile already exists: "'
     &     //trim(datfile)//'"'
 ...
 210  close (outn)
      ...
      subroutine writeinfo(infofile,desc,part,flav,auth,ref,index
     &     ,ver,npdfpar)
*     Writes LHAPDF info file
*     INPUT:  infofile  = (ch*50)  file name
*             desc      = (ch*100) set description
*             part      = (i) hadron PDG index (see below)
*             flav      = (ch*100) included flavor codes (see below)
*             index     = (i) pdf set index   (99999 = generic)
*             iver      = (i) version number  (-1 = not final)
*     All the rest is passed in common, or hard coded for now
*
*     Hadron PDG codes:
*     ----------------
!     part = hadron to which the PDFs belong, see
...
!     However, the proton, neutron should follow the hadron scheme
*
*     Flavor codes
*     ------------
*     flav = string with "flavor codes" in [f1, f2, f3, ..., fn] format
*     Example:  [-5, -4, -3, -2, -1, 1, 2, 3, 4, 5, 21]
*                bb, cb, sb, ub, db, u, d, s, c, b,  g
*     Note:
*     Codes 901-920 can be used for "PDF purposes", see e-mail
*     exchange with T.Sjostrand and PDG developers. This mean whatever
*     extra information a PDF fit collaboration may want to add.
*     For CJ purposes, we can use these to distribute structure functions.
*     Since these numbers are non-standard, amke sure to describe them
*     in the dec field
*     
      implicit none

      character*50 infofile
      character*1000 desc,auth,ref,flav
      integer part,index,ver,npdfpar
...
*    *** Info for .info file
      ...
      nu = NextUn()
      open(unit=nu,file=trim(infofile),status='new',err=200)

      write(nu,'(A)') "SetDesc: '"//trim(desc)//"'"
      write(nu,'(A,I5)') "SetIndex: ", index
      write(nu,'(A)') "Authors: "//trim(auth)
      write(nu,'(A)') "Reference: "//trim(ref)
      write(nu,'(A)') "Format: lhagrid1"
      write(nu,'(A,I2)') "DataVersion: ", ver
      write(nu,'(A,I3)') "NumMembers: ",2*npdfpar+1
      write(nu,'(A,I8)') "Particle: ",part !2212
      write(nu,'(A,A)') "Flavors: ",trim(flav) ![-5, -4, -3, -2, -1, 1, 2, 3, 4, 5, 21]"
      write(nu,'(A,I1)') "OrderQCD: ", ord
      write(nu,'(A)') "ForcePositive: 1"
      if (hq.eq.0) then
         label = "variable"
      else if (hq.eq.1) then
         label = "variable"
      end if
      write(nu,'(A,A)') "FlavorScheme: ", trim(label)
      write(nu,'(A,I3)') "NumFlavors: ", nfl
      write(nu,'(A)') "ErrorType: hessian"
      write(nu,'(A)') "ErrorConfLevel: 90.0"
      write(nu,'(A,es12.2)') "XMin: ", xmin
      write(nu,'(A,es12.2)') "XMax: ", xmax
      write(nu,'(A,es12.2)') "QMin: ", Qmin
      write(nu,'(A,es12.2)') "QMax: ", Qmax
      write(nu,'(A,f10.4)') "MZ: ", MZ
      write(nu,'(A,f3.1)') "MUp: ", qm1
      write(nu,'(A,f3.1)') "MDown: ", qm2
      write(nu,'(A,f3.1)') "MStrange: ", qm3
      write(nu,'(A,f3.1)') "MCharm: ", qm4
      write(nu,'(A,f3.1)') "MBottom: ", qm5
      write(nu,'(A,f5.1)') "MTop: ", qm6
      write(nu,'(A,f8.6)') "AlphaS_MZ: ", alphas_MZ
      write(nu,'(A,I2)') "AlphaS_OrderQCD: ", ord
      write(nu,'(A)') "AlphaS_Type: ipol"
      comma = ', '
      write(nu,'(A,es12.6,100(A,es12.6),A)')
     &     "AlphaS_Qs: [",
     &     alphas_Q(1),(comma,alphas_Q(i),i=2,nalpha)
     &     ,"]"
      write(nu,'(A,es12.6,100(A,es12.6),A)')
     &     "AlphaS_Vals: [",
     &     alphas_v(1),(comma,alphas_v(i),i=2,nalpha)
     &     ,"]"
      write(nu,'(A,f5.3)') "AlphaS_Lambda4: ", alphas_L4
      write(nu,'(A,f5.3)') "AlphaS_Lambda5: ", alphas_L5
      
      goto 210
 200  print*, 'WARNING(writeinfo): infofile already exists: "'
     &     //trim(infofile)//'"'
 210  close(nu)
...
*    ... sets parameters for evolution 
      ...
      ! Initialization of Nobuo's alphas routine
      call set_alphaSMZ(par(1))
      ...
      ! PDF evolution
      CALL INTQCD(PAR)          ! evolves the PDFs
      call fillgf 

      ! set offshell paramaters
      call setoffshell(0,xc) ! sets / calculates offshell constants
