...
c      fcnvl=xp(31)*fubpdb(x)*(1.-x)**xp(34)-.25*fcnfs(x)
         if(iabs(iparamc).ne.8)then
            fcnvl=xp(31)*fubpdb(x)-.25*fcnfs(x)
         else if(iabs(iparamc).eq.8)then
            fcnvl=xp(31)*x**xp(7)*(1.-x)**xp(13)*fubpdb(x)-.25*fcnfs(x)
         endif
c
...
c
c  iparamc=8 frees up xp(7) for use in strange sea studies -- jfo 11/26/16
c
      else if(iabs(iparamc).ge.3.and.iabs(iparamc).lt.8)then
         fuv=xp(2)*x**xp(3)*(1.-x)**xp(4)*(1.+xp(5)*sqrt(x)+xp(6)*x
     &       +xp(7)*x*sqrt(x))
      endif  
      return
...
C     ### (in the future may want to use squares insted of abs values)
     ...
     &      + const*x**alpha*fuv(x))
c
c  iparamc=8 frees up xp(13) for strange sea studies -- jfo 11/26/16
c
      else if(iabs(iparamc).eq.8)then
         fdv=xp(8)*(x**xp(9)*(1.-x)**xp(10)*(1.+xp(11)*sqrt(x)+xp(12)*x)
     &        + const*x**alpha*fuv(x))
...
c
      ...
      else if (iabs(iparamc).eq.7.or.iabs(iparamc).eq.8)then
         fdboub=xp(20)*x**(xp(21))*(1.-x)**xp(22)
c     $   +(1.+xp(23)*x)*(1.-x)**xp(24)
     $   +1.+xp(23)*x*(1.-x)**xp(24)
...
c  singlet at Q0
c
c      fcnfs=fuv(x)+fdv(x)+fubpdb(x)*(2.+xp(31)*(1.-x)**xp(34))
      fcnfs=fuv(x)+fdv(x)+fubpdb(x)*(2.+xp(31))
c
c  jfo - removed strangeness suppression factor as xp(34) is used elsewhere
c
      if(iabs(iparamc).eq.8)then
         fcnfs=fuv(x)+fdv(x)+fubpdb(x)*(2.+
     $         xp(31)*x**xp(7)*(1.-x)**xp(13))
      endif
      return
      end
