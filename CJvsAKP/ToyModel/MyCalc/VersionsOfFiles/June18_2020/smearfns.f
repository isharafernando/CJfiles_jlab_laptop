...
C ***********************************************************************
      ...
      REAL*8  FYINT(4),FYOFF(4),ymax,pT,pTmin,pTmax,pTlim,pTint
     %     ,fyp0(3),fyp1(3)
...
C...Kinematic limits
      ...
      pTmin = 0.D0
      pTmax = 1.D0 * ipv        ! INTEGRATION LIMITS (pT<pv_cut)
      npT = 2000

      if (rel.eq.0) then        ! WBA (non relativistic)
         ymax  = (mN/MD)*(1.D0 + gamma**2/2 + epsD/mN) ! not exact
         ptmax = dsqrt(2*mN**2*(-MD/mN*y+1+gamma**2/2+epsD/mN)) ! adaptive grids for pT
      ...
      pTmin = 0.D0
C..      pTmax = 2000.D0           ! INTEGRATION LIMITS
C..      npT = 2000
      pTmax = 2000.D0           ! Changed by Ishara to check what happens
      npT = 2000	        ! Changed by Ishara to check what happens
...
*     rel      = (i) relativ. corrections 0=WBA  1=WBAREL  2=AQV
*     y        = (dp) nucleon fractional momentum
*                     NOTE: y=p.q/PD.q with PD the unrescaled Deuteron momentum
*     gamma    = (dp) gamma factor (NOTE: not gamma^2)
...
C *********************************************************************
      ...
      REAL*8 ymax,y0,pz,pv,pv_cut,Ep,p0,p2,eps
...
C...Separation energy
        eps = MD - Ep - mN  ! valid for both rel and non-rel options
!      ... OLD CODE, REMOVE AFTER TESTINg (AA: 15 June 2020)
!       IF (rel.eq.0) THEN
!	  eps = epsD - pv**2/(2*mN)
!	ELSE
!	  eps = MD - Ep - mN
...
C...Added commandline flag to vary pv_cut values--SL, 05.2020
	pv_cut = ipv !1200.D0 
...
C...Calculates (u^2+v^2+w^2) = |psi|^2 / (4\pi)
C...(--> the 1/(4\pi) term is included in the Jacobian below)  
...
C...  WBA at finite Q^2
      ...
      ! old Jacobian: <= May 2020
      ! -- bugged?  (mN/MD)/2 --> 1/4 ; origin of remaining part unclear
      ! Jac = gamma
      ! prefact =  Ep*mN/MD / (2*Jac*(1.D0-y0)) ! Jacobian etc.

      ! New Jacobian (15 June 2020) agrees with new calcs and AKP
      ! d3p -> dy*dpT2 & normalization of |u^2+v^2| read in from file
      !prefact =  gamma*mN*Ep / (4*MD*(1-y)+(gamma**2-1)*Ep)   v1
       prefact =  gamma*Ep / 4*((1-y)+(gamma**2-1)*Ep/MD)   
      !prefact =  gamma*Ep / (4*(1-y)+(gamma**2-1)*Ep) v1p
