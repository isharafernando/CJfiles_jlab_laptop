...
C ***********************************************************************
	IMPLICIT NONE
        ...
        REAL*8	y,yint,yplot(0:iny),fy0(4,0:iny),fy1(4,0:iny)
	...
	character model*7,pot*20,cut*5,cdum*100,outfil*30,uflag*100
...
C...Read commandline flags
      ...
      do
         ...
         if (uflag.eq.'') then
            ...
            call getarg(nfg,uflag)
            read (uflag,*) ipv
            print*, 'WARNING -- PV_CUT SET TO ',ipv,' MeV'
...
c...(and exits because of intended 'end of file' error)
...
c...    Attention: overwrites previous files
	...
	if (rel.eq.0) then
	   if (ibar.eq.0) then 
	      ...
	      write(*,*) 'ERROR(smeargrids): ibar out of bounds '
     &	        //'for chosen rel',ibar,rel
	      stop
	   end if
	...
	write(11,*) 'Smearing function S(yD,gamma) in 1D '//trim(model)
     &       //' approximation, finite-Q^2'
	write(cut,'(I5)') ipv
	write(11,*) 'Uses '//trim(pot)//' potential '
     &       //'with Pv_cut = ',trim(adjustl(cut)),' MeV'
	write(11,*) 'Ref: Kahn et al.arXiv:0809.4308 '
     &       //'+ notes by Melnitchouk and Accardi 2010-2011 '
     &       //'and Accardi 2020'
...
C...   ...Cycles over nucleon yD momentum fraction
          norm0 = 0.D0
          norm1 = 0.D0
          yint = 1/DFLOAT(ny)
	  DO iy=1,ny-1
